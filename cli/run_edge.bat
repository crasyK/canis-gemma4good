...
if %ERRORLEVEL% equ 0 (
  ...
  wt -w 0 new-tab --title "Canis pipelines" -d "%CLI_DIR%" cmd /k ".\.venv\Scripts\python.exe -m canis.interfaces.server --host 0.0.0.0 --port %CANIS_API_PORT%"
) else (
  ...
  timeout /t 20 /nobreak >nul
  start "Canis API" cmd /k ".\.venv\Scripts\python.exe -m canis.interfaces.server --host 0.0.0.0 --port %CANIS_API_PORT%"
)
